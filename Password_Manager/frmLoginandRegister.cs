...
namespace Password_Manager
{
    public partial class frmLoginandRegister : Form
    {
        ...
        private void btnAccountRegister_Click(object sender, EventArgs e)
        {
            if (isAccept())
            {
                ...
                try
                {
                    ...
                    MessageBox.Show(this, "Bạn đã đăng ký thành công", "Success");
                    bunifuPages1.SetPage(0);
                }
                catch (Exception ex)
                {
        ...
        private void txtRegisterAccount_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                errorProvider1.SetError(txtRegisterAccount, "Vui lòng nhập username");
                accept[0] = false;
            }
            else if (txtRegisterAccount.Text.Length > 1000)
            {
                errorProvider1.SetError(txtRegisterAccount, "username quá dài");
                accept[0] = false;
            }
            else if (!Regex.IsMatch(txtRegisterAccount.Text, "^[a-zA-Z0-9]+$"))
            {
                errorProvider1.SetError(txtRegisterAccount, "username không bao gồm khoảng trắng và ký tự đặc biệt");
                accept[0] = false;
            }
            else
            {
        ...
        private void txtRegisterPassword_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                errorProvider1.SetError(txtRegisterPassword, "Mật khẩu không được trống");
                accept[1] = false;
            }
            else
            {
        ...
        private void txtConfirmRegisterPassword_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                errorProvider1.SetError(txtConfirmRegisterPassword, "Mật khẩu không khớp");
                accept[2] = false;
            }
            else
            {
        ...
        private void txtRegisterPhoneNumber_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                errorProvider1.SetError(txtRegisterPhoneNumber, "Số điện thoại không được trống");
                accept[3] = false;
            }
            else if (!Regex.IsMatch(txtRegisterPhoneNumber.Text, "^\\d+$") | txtRegisterPhoneNumber.Text.Length > 13)
            {
                errorProvider1.SetError(txtRegisterPhoneNumber, "số điện thoại không hợp lệ");
                accept[3] = false;
            }
            else
            {
        ...
        private void linkPolicy_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
